...
```
If the pages declare its resources correctly, then all the users being able to access this page (because it is part of a custom profile or REST API extention they have access to) will also be automatically granted the necessary permissions to call the REST API extention. This works the same way as out of the box REST API resources.
